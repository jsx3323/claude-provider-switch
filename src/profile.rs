...
pub fn list_profiles() -> Result<Vec<String>, CsError> {
    ...
    let entries = fs::read_dir(&dir).map_err(|e| io_err(dir.to_str().unwrap_or(""), e))?;
    for entry in entries {
        let entry = entry.map_err(|e| io_err(dir.to_str().unwrap_or(""), e))?;
        let path = entry.path();
...
pub fn read_current(project: &Path) -> Result<Option<String>, CsError> {
    ...
    match fs::read_to_string(&path) {
        ...
        Err(e) => Err(io_err(path.to_str().unwrap_or(""), e)),
    }
}
...
pub fn write_current(project: &Path, name: &str) -> Result<(), CsError> {
    ...
    fs::create_dir_all(dir).map_err(|e| io_err(dir.to_str().unwrap_or(""), e))?;
    fs::write(&path, name).map_err(|e| io_err(path.to_str().unwrap_or(""), e))?;
    Ok(())
}
...
pub fn read_profile(name: &str) -> Result<Value, CsError> {
    ...
    let content = fs::read_to_string(&path).map_err(|e| {
        if e.kind() == std::io::ErrorKind::NotFound {
            ...
            io_err(path.to_str().unwrap_or(""), e)
        }
    })?;
    serde_json::from_str(&content).map_err(|e| json_err(path.to_str().unwrap_or(""), e))
}
...
pub fn save_profile(name: &str, content: &Value) -> Result<(), CsError> {
    ...
    fs::create_dir_all(&dir).map_err(|e| io_err(dir.to_str().unwrap_or(""), e))?;
    let path = profile_path(name);
    let json = serde_json::to_string_pretty(content).map_err(|e| json_err(path.to_str().unwrap_or(""), e))?;
    fs::write(&path, json).map_err(|e| io_err(path.to_str().unwrap_or(""), e))?;
    Ok(())
}
...
pub fn read_settings_local(project: &Path) -> Result<Value, CsError> {
    ...
    let content = fs::read_to_string(&path).map_err(|e| {
        if e.kind() == std::io::ErrorKind::NotFound {
            CsError::SettingsNotFound { path: project.to_str().unwrap_or("").into() }
        } else {
            io_err(path.to_str().unwrap_or(""), e)
        }
    })?;
